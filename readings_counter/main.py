...
for section in txt_input:
    ...
    for subsection in section:
        ...
        pg_cnt[-1].append((sub, page_count))
        total_pages += page_count


prefix = (
    len(f"Pages to read per day ({DAYS[0]}-{DAYS[-1]}):") - len("Total pages to read:")
) * " "

print("== Reading Plan Summary ==\n")

print(prefix + f"Total pages to read: {total_pages:.2f}")
ppd = total_pages / len(DAYS)
print(f"Pages to read per day ({DAYS[0]}-{DAYS[-1]}): {ppd:.2f}")
...
if FILE_OUTPUT:
    with open(os.path.join(os.path.dirname(__file__), OUTPUT_FILE), "w") as out_file:
        ...
        out_file.write(f"Pages to read per day ({DAYS[0]}-{DAYS[-1]}): {ppd:.2f}\n")
...
for day in range(len(DAYS)):
    ...
    while pages_covered < ppd and ss_pointer < len(pg_cnt):
        for sub, page_count in pg_cnt[ss_pointer]:
    ...
    if FILE_OUTPUT:
        with open(
            ...
            out_file.write(
                f"  Total pages for the day: {pages_covered} (Target: {ppd:.2f}) "
